...
class ResConfigSettings(models.TransientModel):
    ...
    bottle_product_id = fields.Many2one('product.product', string='Bottle Product', help='Bottle product used for exchanging coupons')

    def get_values(self):
        res = super(ResConfigSettings, self).get_values()
        ICPSudo = self.env['ir.config_parameter'].sudo()
        coupon_product_id = ICPSudo.get_param('az_coupons.coupon_product_id')
        bottle_product_id = ICPSudo.get_param('az_coupons.bottle_product_id')
        ...
        res.update(
            ...
            bottle_product_id=int(bottle_product_id) if coupon_product_id else False,
    ...
    def set_values(self):
        ...
        ICPSudo.set_param("az_coupons.bottle_product_id", self.bottle_product_id.id)
