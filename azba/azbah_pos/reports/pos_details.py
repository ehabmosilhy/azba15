...
class PosDetails(models.TransientModel):
    ...
    def _default_start_date(self):
        ...
        return (latest_start_dates and min(latest_start_dates) or fields.Datetime.now()).date()

    def generate_report(self):
        c = self.pos_config_ids
        configs = [(p.id, p.name, p.employee_ids.mapped("name")) for p in c]
        data = {'date_start': self.start_date, 'date_stop': self.end_date, 'config_ids': self.pos_config_ids.ids,'configs': configs}
