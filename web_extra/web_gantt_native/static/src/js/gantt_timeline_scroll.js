...
var GanttTimeLineScroll = Widget.extend({
    ...
    start: function(){
        ...
        if (scroll_end_dt == "Invalid Date") {
            $('<div class="timeline-gantt-scroll-scale-start-date">'+"لايوجد تاريخ بداية"+'</div>').appendTo(bar_start);
            $('<div class="timeline-gantt-scroll-scale-end-date">'+"لا يوجد تاريخ نهاية"+'</div>').appendTo(bar_end);
                }
        else{
