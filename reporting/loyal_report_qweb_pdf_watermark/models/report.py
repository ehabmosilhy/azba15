...
class Report(models.Model):
    ...
    def _run_wkhtmltopdf(
        ...
        selected_company_id = self.env.context.get('selected_company_id', False)
        watermark = None
        cmp = self.env['res.company']

        if selected_company_id:
            cmp = self.env['res.company'].browse(selected_company_id[0])
        # if cmp.company_watermark:
        #     watermark = b64decode(cmp.company_watermark)
