...
class ReportReceivablePartnerLedger(models.AbstractModel):
    _name = 'report.ejad_ejad_erp_account.report_receivable_partnerledger'

    def _lines(self, data, partner):
        full_account = []
        currency = self.env['res.currency']
        query_get_data = self.env['account.move.line'].with_context(data['form'].get('used_context', {}))._query_get()
        params = [partner.id, tuple(data['computed']['account_ids'])] + query_get_data[2]
        query = """
        ...
        for r in res:
            ll_b = ml_obj.browse(r['id']).payment_id
            ...
            r['br'] = ll_b.bank_ref or ''
            r['statement'] = (ll_b.communication or '') + ' ' + (ll_b.note or '')
    ...
    def _get_report_values(self, docids, data=None):
        ...
        self.env.cr.execute("""
            ...
            WHERE a.internal_type = %s
            AND NOT a.deprecated""", (tuple(data['computed']['ACCOUNT_TYPE']),))
        data['computed']['account_ids'] = [a for (a,) in self.env.cr.fetchall()]
        params = [tuple(data['computed']['account_ids'])] + query_get_data[2]
