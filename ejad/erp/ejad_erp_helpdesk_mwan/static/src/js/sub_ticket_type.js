odoo.define('ejad_erp_helpdesk.sub_type_ticket', function (require) {
    ...
    var ajax = require('web.ajax');
        ...
        $('#ticket_type_id').change(function(){
            ...
            .then(function (data) {
                  // Action after update
                  var part1 = ' <label class="col-form-label" for="ticket_type_line_id">Sub Type</label>
                                 <select type="text" class="form-control o_website_form_input" id="ticket_type_line_id" name="ticket_type_line_id" required="1">'
                  var html = "";
                  for(var x in data){
                    var obj = data[x]
                    for(var key in obj){
